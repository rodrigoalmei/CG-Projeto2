...
function applyComposition(matrixA, matrixB, w, h, operatorFn, doNormalize = true) {
    const result = [];
    let minVal = Infinity;
    let maxVal = -Infinity;
...
filterUpload.addEventListener('change', async (event) => {
    ...
    try {
        ...
        filterInspectors.original.setImage({
            ...
            pixels: matrixToLinearArray(filterImageState.data, filterImageState.w, filterImageState.h)
        });
...
applyFilterBtn.addEventListener('click', () => {
    ...
    setTimeout(() => {
        ...
        if (isNaN(filterValue) || filterValue === 0) {
            ...
            result = highBoostFilter(data, w, h, parseFloat(highBoostInput.value), filterNormalize.checked);
...
downloadFilterBtn.addEventListener('click', () => {
    ...
    const blob = matrixToPGM(filterProcessedMatrix, filterImageState.w, filterImageState.h);
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
